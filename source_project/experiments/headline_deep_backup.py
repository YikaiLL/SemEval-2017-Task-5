...
from keras.layers import Dense, Activation, Embedding, Convolution1D, Dropout, recurrent, LSTM, GRU
...
def pack_data_to_format():
    ...
    print(len(pred_list))
    json.dump(pred_list,
              open('/raid/data/skar3/semeval/source/ml_semeval17/outputs/subtask2_hl/dl_submission2.json', 'w'))
...
def cnn_merged():
    ...
    combined_model.add(Concatenate([model_1, model_2, model_3, model_4], concat_axis=1))
...
def attention_imp_merge():
    ...
    merged_layer.add(Concatenate([model_atn, model_feature_vec],
...
def attention_imp_merge_exp_e():
    ...
    merged_layer = Sequential()
    merged_layer.add(Concatenate([model_atn, model_feature_vec, model_cnn],
                           concat_axis=1, name='merge_layer'))
...
def attention_imp_merge_exp():
    ...
    merged_cnn = Concatenate([cnn(sentence_emb) for cnn in cnn_layers], concat_axis=-1)
